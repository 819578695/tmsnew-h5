...
export default {
  data(){
    return {
      ...
      count:0,
      disabled: false,
      date:{befor:new Date(),after: new Date()},
      dateParemes:{
        now: new Date(),
        show: false,
        minDate: new Date(),
  ...
  mounted(){
    var this_=this;
     //查询开票报表合计数
    this.utils.request.getAllTotal({status: 0,t:'preservation-company'},function(data){
       if(null!=data){
    ...
    //查询开票列表数据
    this.utils.request.getAllList({status: 0,t:'preservation-company',limit:'20'},function(data){
       var array=[];
       this_.dataList=data;
       if(null!=data){
         for(var i=0;i<20;i++){
  ...
  methods:{
    ...
    confirmDatetime(val){
      ...
      this.getLoadLists({begin_time:this.utils.dateTransion(this.date.befor,1),end_time:this.utils.dateTransion(this.date.after,1),trader_id:this.actionValId,status: 0,t:'ticket',limit:'20'})
      this.toggleDatetime()
    },
    getLoadLists(req){
       //查询保全列表数据
